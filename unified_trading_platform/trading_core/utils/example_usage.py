...
from trading_core.utils import (
    ...
    # API Client
    create_trading_api_client,
    # WebSocket Client
    create_websocket_client,
    create_trading_websocket_client,
    WebSocketMessage,
)
...
def example_rest_client():
    ...
    client = create_rest_client(
        base_url="https://api.example.com", auth_manager=auth, rate_limiter=rate_limiter, timeout=30, max_retries=3
    )
    ...
    try:
        ...
        updated_user = {"name": "John Smith", "email": "john.smith@example.com", "age": 31, "address": "123 Main St"}
...
def example_trading_api_client():
    ...
    trading_client = create_trading_api_client(
        base_url="https://api.trading.com", auth_manager=auth, rate_limiter=rate_limiter
    )
    ...
    try:
        ...
        # Submit order
        order_data = {"symbol": "AAPL", "quantity": 100, "side": "buy", "type": "limit", "price": 150.00}
        ...
        # Get historical data
        response = trading_client.get_historical_data("AAPL", "2024-01-01", "2024-01-31", "1d")
