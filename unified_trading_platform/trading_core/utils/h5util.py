    ...
    def read_futures_series(self, symbol: str = "NIFTY", expiry: Optional[str] = None) -> pd.DataFrame:
        ...
        if expiry:
            possible_paths.extend(
                [f"/futures/{symbol}/{expiry}", f"/futures_data/{symbol}/{expiry}", f"/{symbol}/futures/{expiry}"]
            )
...
def create_h5_from_dataframes(
    dataframes: Dict[str, pd.DataFrame], output_path: Union[str, Path], compression: str = "gzip"
) -> None:
    """
...
def convert_h5_to_csv(h5_file_path: Union[str, Path], dataset_path: str, output_csv_path: Union[str, Path]) -> None:
    """
