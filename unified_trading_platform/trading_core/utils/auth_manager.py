...
class HMACAuth(AuthManager):
    ...
    def get_auth_headers(self) -> Dict[str, str]:
        ...
        return {self.header_name: self.api_key, self.signature_header: signature, self.timestamp_header: timestamp}
...
def create_oauth2_auth(client_id: str, client_secret: str, token_url: str, scope: Optional[str] = None) -> OAuth2Auth:
    """Create OAuth 2.0 authentication"""
    return OAuth2Auth(client_id, client_secret, token_url, scope)


def create_hmac_auth(
    api_key: str, secret_key: str, header_name: str = "X-API-Key", signature_header: str = "X-Signature"
) -> HMACAuth:
