...
class WebSocketClient:
    ...
    async def _connect_with_tenacity(self):
        ...
        retry_decorator = AsyncRetrying(
            stop=stop_strategy,
            wait=wait_exponential(multiplier=self.reconnect_delay, min=1, max=self.max_reconnect_delay),
            retry=retry_if_exception_type((ConnectionClosed, InvalidHandshake, InvalidURI, OSError, ConnectionError)),
            reraise=True,
        )
