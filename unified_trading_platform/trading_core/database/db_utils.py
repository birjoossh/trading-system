    ...
    with sqlite3.connect(db_path) as conn:
        ...
        """,
            (
                ...
                start_date,
                end_date,
                json.dumps(initial_portfolio or {}),
                "INITIAL",
                exit_time,
                timestamp,
                timestamp,
...
def save_portfolio_snapshot(db_path: str, run_id: str, positions: List[Dict], cash_balance: float, total_value: float):
...
def save_pnl_snapshot(
    ...
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (pnl_id, run_id, timestamp, realized_pnl, unrealized_pnl, total_pnl, num_trades, win_count, loss_count),
        )
