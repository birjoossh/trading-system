...
def main():
    ...
    paper_config = settings.get_broker_config("paper_broker")
    import os
    ...
    success = trading_system.add_broker(name="paper", broker_type="paper", **paper_config)
    if not success:
    ...
    try:
        ...
        option_chain = trading_system.get_option_chain(
            "paper",
            contract=Contract(symbol="NIFTY", exchange="NSE", expiry="2024-01-04", security_type=SecurityType.STOCK),
        )
        if option_chain:
            logger.info("Retrieved option chain:")
            logger.info(f"\n{option_chain}")
        ...
        trading_system.subscribe_market_data(
            symbol="NIFTY 50", exchange="NSE", callback=on_market_data, broker_name="paper"
        )
        # time.sleep(100)
        ...
        buy_order_id = trading_system.submit_limit_order(
            symbol="NIFTY 50", exchange="NSE", action="BUY", quantity=100, limit_price=limit_price, broker_name="paper"
        )
